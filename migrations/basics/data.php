<?php
/**
 * bbGuild FFXIV plugin - Data seeding migration
 *
...
class data extends \phpbb\db\migration\container_aware_migration
{
	...
	public function effectively_installed()
	{
		...
		$sql = 'SELECT COUNT(*) AS cnt FROM ' . $games_table . " WHERE game_id = 'ffxiv'";
	...
	public function seed_game_data()
	{
		...
		$installer->install($this->get_table_names(), 'ffxiv', 'Final Fantasy XIV', '', '', 'us');
	}

	public function remove_game_data()
	{
		$installer = $this->get_installer();
		$installer->uninstall($this->get_table_names(), 'ffxiv', 'Final Fantasy XIV');
	}
